...
export default function SocialMediaIntelligence() {
  ...
  }, [domain, data, competitorDomain]);

  return (
    <div style={{ maxWidth: 1000, margin: "0 auto", padding: "32px 24px 60px", fontFamily: FONT }}>
      ...
      {data && (
        <Card style={{ padding: 20, marginBottom: 16 }}>
          <div style={{ fontSize: 14, fontWeight: 700, color: C.text, marginBottom: 16 }}>
            Social-Media-Profile für {data.domain}
          </div>
          <SocialPlatformGrid profiles={data.profiles} metrics={data.metrics} />

          {data && (
            <div style={{ marginTop: 20, display: "flex", gap: 10 }}>
              <div style={{ flex: 1, display: "flex", alignItems: "center", gap: 10, padding: "8px 12px", borderRadius: T.rMd, border: `1px solid ${C.border}`, background: C.bg }}>
                <input
                  value={competitorDomain}
                  onChange={e => setCompetitorDomain(e.target.value)}
                  onKeyDown={e => e.key === "Enter" && !compLoading && handleCompare()}
                  placeholder="Wettbewerber-Domain zum Vergleich"
          ...
          {compError && (
            <div style={{ marginTop: 8, fontSize: 12, color: "#dc2626" }}>{compError}</div>
          )}
        </Card>
      )}
      ...
      {insightsLoading && (
        <Card style={{ padding: 20, textAlign: "center" }}>
          <div style={{ fontSize: 13, color: C.textSoft }}>KI-Insights werden generiert…</div>
        </Card>
      )}

      {insights && (
        <InsightsPanel insights={insights} domain={domain} compDomain={competitorDomain} compData={compData} />
      )}
